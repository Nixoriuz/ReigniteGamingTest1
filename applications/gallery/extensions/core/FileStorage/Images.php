...
class _Images
{
	...
	public function move( $offset, $storageConfiguration, $oldConfiguration=NULL )
	{
		...
		if( $image['image_thumb_file_name'] )
		{
			try
			{
				$file = \IPS\File::get( $oldConfiguration ?: 'gallery_Images', $image['image_thumb_file_name'] )->move( $storageConfiguration );
				$update['image_thumb_file_name']	= (string) $file;
			}
			catch( \Exception $e )
			{
		...
		if( $image['image_medium_file_name'] )
		{
			if( $image['image_medium_file_name'] == $image['image_masked_file_name'] AND isset( $update['image_masked_file_name'] ) )
			{
				$update['image_medium_file_name']	= $update['image_masked_file_name'];
			}
			else
			{
				try
				{
					$file = \IPS\File::get( $oldConfiguration ?: 'gallery_Images', $image['image_medium_file_name'] )->move( $storageConfiguration );
					$update['image_medium_file_name']	= (string) $file;
				}
				catch( \Exception $e )
				{
					/* Any issues are logged */
	...
	public function fixUrls( $offset )
	{
		...
		foreach( array( 'image_thumb_file_name', 'image_small_file_name', 'image_masked_file_name', 'image_medium_file_name', 'image_original_file_name' ) as $key )
		{
			if( $image[ $key ] )
			{
	...
	public function isValidFile( $file )
	{
		try
		{
			$record	= \IPS\Db::i()->select( '*', 'gallery_images', array( 'image_masked_file_name=? OR image_medium_file_name=? OR image_original_file_name=? OR image_thumb_file_name=? OR image_small_file_name=?', (string) $file, (string) $file, (string) $file, (string) $file, (string) $file ) )->first();
	...
	public function delete()
	{
		foreach( \IPS\Db::i()->select( '*', 'gallery_images' ) as $image )
		{
			if( $image['image_masked_file_name'] )
			{
				try
				{
					\IPS\File::get( 'gallery_Images', $image['image_masked_file_name'] )->delete();
				}
				catch( \Exception $e ){}
			}
			...
			if( $image['image_original_file_name'] )
			{
				try
				{
					\IPS\File::get( 'gallery_Images', $image['image_original_file_name'] )->delete();
				}
				catch( \Exception $e ){}
			}
			...
			if( $image['image_small_file_name'] )
			{
				try
				{
					\IPS\File::get( 'gallery_Images', $image['image_small_file_name'] )->delete();
				}
				catch( \Exception $e ){}
			}
		}
