...
class _incomingemail extends \IPS\Dispatcher\Controller
{
	...
	{	
		...
		if ( $values = $form->values() )
		{
			...
			try
			{
				if ( !$pop3->connect( ( $values['pop3_tls'] ? 'ssl://' : '' ) . $values['pop3_server'], $values['pop3_port'] ) )
				{
					throw new \DomainException( \IPS\Member::loggedIn()->language()->addToStack('pop3_connect_err') );
				}

				if ( !$pop3->login( $values['pop3_user'], $values['pop3_password'] ) )
				{
					throw new \DomainException( \IPS\Member::loggedIn()->language()->addToStack('pop3_login_err') );
				}
