...
class _AuthorizeNet extends \IPS\nexus\Gateway
{
	...
	public function auth( \IPS\nexus\Transaction $transaction, $values, \IPS\nexus\Fraud\MaxMind\Request $maxMind = NULL )
	{
		...
		if ( $card instanceof \IPS\nexus\Gateway\AuthorizeNet\CreditCard )
		{
			...
			$xml = $card->create( 'createCustomerProfileTransactionRequest' );
		...
		else
		{
			...
			$response = $this->api( array(
				'x_type'			=> 'AUTH_ONLY',
				'x_method'			=> 'CC',
				'x_amount'			=> $transaction->amount->amount,
	...
	public function refund( \IPS\nexus\Transaction $transaction, $amount = NULL )
	{
		...
		$this->api( array(
			'x_type'		=> 'CREDIT',
			'x_trans_id'	=> $transaction->gw_id,
			'x_amount'		=> $amount ?: $transaction->amount->amount,
