...
class _Feed extends \IPS\Node\Model
{
	...
	public function run()
	{
		...
		if( !in_array( $request->getname(), array( 'feed', 'rss' ) ) )
		{
		...
		foreach ( $request->articles( $this->id ) as $guid => $article )
		{
			if ( !in_array( $guid, $previouslyImportedGuids ) )
			{
				...
				$content = \IPS\Text\Parser::parseStatic( $article['content'] . $readMoreLink, FALSE, NULL, $member, 'forums_Forums', TRUE, !(bool) $member->group['g_dohtml'] );
