...
class _view extends \IPS\Content\Controller
{
	...
	protected function rate()
	{
		...
		try
		{
			$existingRating = \IPS\Db::i()->select( '*', 'nexus_support_ratings', array( 'rating_reply=?', $reply->id ) )->first();
		}
		...
		$form->add( new \IPS\Helpers\Form\TextArea( 'support_rating_feedback', $existingRating ? $existingRating['rating_note'] : NULL, FALSE ) );
		if ( $values = $form->values() )
		{
			...
			if ( $existingRating )
			{
				\IPS\Db::i()->update( 'nexus_support_ratings', $save, array( 'rating_reply=?', $reply->id ) );
			}
			else
			{
