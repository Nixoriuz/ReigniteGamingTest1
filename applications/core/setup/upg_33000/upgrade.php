...
class _Upgrade
{
	...
	public function step7()
	{
		...
		if( \IPS\Db::i()->select( 'count(*)', 'posts' )->first() > 100000 )
		{
			...
			{
				$queries[]	= "ALTER TABLE " . \IPS\Db::i()->prefix . "posts ADD post_field_int INT(10) NOT NULL DEFAULT 0;";
			}
			
			if ( !\IPS\Db::i()->checkForColumn( 'posts', 'post_field_t1' ) )
			{
				$queries[]	= "ALTER TABLE " . \IPS\Db::i()->prefix . "posts ADD post_field_t1 TEXT NULL DEFAULT NULL;";
			}
			
			if ( !\IPS\Db::i()->checkForColumn( 'posts', 'post_field_t2' ) )
			{
				$queries[]	= "ALTER TABLE " . \IPS\Db::i()->prefix . "posts ADD post_field_t2 TEXT NULL DEFAULT NULL;";
			}
			...
			return array( 'html' => \IPS\Theme::i()->getTemplate( 'forms' )->queries( $queries, \IPS\Http\Url::internal( 'controller=upgrade' )->setQueryString( array( 'key' => $_SESSION['uniqueKey'], 'mr_continue' => 1, 'mr' => $mr ) ) ) );
		}

		if ( !\IPS\Db::i()->checkForColumn( 'posts', 'post_field_int' ) )
		{
			\IPS\Db::i()->addColumn( 'posts', array(
				'name'			=> 'post_field_int',
				'type'			=> 'int',
				'length'		=> 10,
				'allow_null'	=> false,
