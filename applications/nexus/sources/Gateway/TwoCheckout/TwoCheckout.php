...
class _TwoCheckout extends \IPS\nexus\Gateway
{
	...
	public function auth( \IPS\nexus\Transaction $transaction, $values, \IPS\nexus\Fraud\MaxMind\Request $maxMind = NULL )
	{
		...
		if ( $transaction->amount->amount == $transaction->invoice->total->amount )
		{
			...
			{
				$data["li_{$k}_type"] = $item->price->amount >= 0 ? 'product' : 'coupon';
				$data["li_{$k}_name"] = $item->name;
				$data["li_{$k}_quantity"] = $item->quantity;
				$data["li_{$k}_price"] = abs( $item->price->amount );
			...
			foreach ( $summary['shipping'] as $shipping )
			{
				...
				$data["li_{$k}_price"] = $shipping->price->amount;
			...
			foreach ( $summary['tax'] as $tax )
			{
				...
				$data["li_{$k}_quantity"] = 1;
				$data["li_{$k}_price"] = $tax['amount']->amount;
		...
		else
		{
			...
			$data["li_0_price"] = $transaction->amount->amount;
	...
	public function refund( \IPS\nexus\Transaction $transaction, $amount = NULL )
	{
		...
		$settings = json_decode( $this->settings, TRUE );
		$response = \IPS\Http\Url::external( 'https://www.2checkout.com/api/sales/refund_invoice' )->request()->login( $settings['api_username'], $settings['api_password'] )->post( $data )->decodeJson();
	...
	{		
		$response = \IPS\Http\Url::external( 'https://www.2checkout.com/api/acct/detail_company_info' )->request()->setHeaders( array( 'Accept' => 'application/json' ) )->login( $settings['api_username'], $settings['api_password'] )->get()->decodeJson();
		if ( isset( $response['errors'] ) )
		{
			throw new \LogicException( $response['errors'][0]['message'] );
		}
		if ( $response['response_code'] != 'OK' )
		{
			throw new \LogicException( $response['response_message'] );
