...
{	
	...
	{
		$classname = 'IPS\nexus\Gateway\\' . $data['po_gateway'] . '\\Payout';
	 ...
	 */
	public static function table( $where = array(), \IPS\Http\Url $url )
	{
		...
		$table->parsers = array(
			...
			{
				return \IPS\Theme::i()->getTemplate('payouts')->status( $val );
			},
		...
		if ( $this->status === static::STATUS_PENDING )
		{
			if( \IPS\Member::loggedIn()->hasAcpRestriction( 'nexus', 'payments', 'payouts_process' ) )
			{
				$return['approve'] = array(
					'title'		=> 'approve',
					'icon'		=> 'check',
					'link'		=> $url->setQueryString( 'do', 'process' ),
					'data'		=> array( 'confirm' => '' )
				);
			}
