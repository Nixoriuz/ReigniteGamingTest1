...
class _search extends \IPS\Dispatcher\Controller
{
	...
	{
		$table = new \IPS\Helpers\Table\Db( 'nexus_customers', \IPS\Http\Url::internal('app=nexus&module=customers&controller=search') );
		...
		$table->include = array( 'photo', 'cm_last_name', 'cm_first_name', 'email', 'address' );
		$table->parsers = array(
		...
		$table->rowButtons = function( $row )
		{
			return array(
				'view'	=> array(
					'icon'	=> 'search',
					'title'	=> 'view',
					'link'	=> \IPS\Http\Url::internal("app=nexus&module=customers&controller=view&id={$row['member_id']}"),
				),
			);
		};
